...
class MemoryManager:
    """Gestor de memoria persistente y asíncrono por usuario."""
    ...
    async def load_context(self, user_id: str) -> Dict[str, Any]:
        """Carga el contexto del usuario de forma asíncrona."""
    ...
    async def save_context(self, user_id: str, context: Dict[str, Any]) -> None:
        """Guarda el contexto completo del usuario de forma asíncrona."""
        path = self._get_user_path(user_id)
        try:
    ...
    async def clear_context(self, user_id: str):
        """Elimina el contexto de un usuario de forma asíncrona."""
        path = self._get_user_path(user_id)
        try:
