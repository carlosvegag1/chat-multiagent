...
type Convo = { convo_id: string; created_at: string };

export default function ChatV1_2() {
  ...
  // 🔹 Al cargar: comprobar nombre o pedirlo
  useEffect(() => {
  ...
  const sendText = async () => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { role: "bot", text: "[Error de conexión con el servidor]" },
      ]);
    }
  ...
  const startRecording = async () => {
    try {
      ...
      mr.onstop = async () => {
        ...
        setMessages((prev) => [...prev, { role: "user", text: "(voz — enviando...)" }]);
        ...
        try {
          ...
          const data = await res.json();
          setMessages((prev) => [
            ...prev.filter((m) => m.text !== "(voz — enviando...)"),
  ...
  const createNewConvo = async () => {
    ...
    const newConvo = await res.json();
    setAvailableConvos((prev) => [newConvo, ...prev]);
    setConvoId(newConvo.convo_id);
    setMessages([]);
  };
          ...
          <div key={i} className={`flex ${m.role === "user" ? "justify-end" : "justify-start"} animate-fade-in`}>
            <div className="flex items-start gap-2">
              ...
              >
                ...
                {m.timestamp && (
                  <div className={`text-xs mt-1 text-right ${m.role === "user" ? "text-white/80" : "text-slate-500"}`}>
                    {m.timestamp}
                  </div>
                )}
              </div>
